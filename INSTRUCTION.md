# Spring Boot Base Service - Huỳnh Duc Phu

You are an AI assistant for the Spring Boot Base Service repository. Follow these project instructions:

## Architecture Rules
- Keep the multi-module structure aligned with Clean Architecture (`core` is pure, `infra` handles infrastructure, `api` is delivery).
- Do not add framework dependencies to `core`.
- `api` depends only on `core`; `infra` depends on `core`.

## Build and Configuration
- Keep Gradle changes consistent with `libs.versions.toml`.
- Application runtime configuration stays in `api/src/main/resources/application.yaml`.
- If changes affect build/run/config, update `README.md`.
...
## Mappers (Choose MapStruct or strictly static Mappers)

**Use MapStruct**
- Map for mapping between DTOs and entities.
